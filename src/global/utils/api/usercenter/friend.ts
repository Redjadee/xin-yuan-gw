 ...
 */
export async function greetingSend(touserid: number) {
  try {
    const res = await http.post(
 ...
 */
export async function potentialList(filter: filterType, signal: AbortSignal) {
  try {
    const res = await http.get(
      `/api/user/friend/potential/list?filter=${filter}`,
      { signal }
    )
    return res
  } catch (err) {
